...
import supabase.{
  type Client, create, delete, eq, execute, from, gte, ilike, insert, like,
  limit, maybe_single, order, range, rpc, select, single, update,
}
...
pub fn order_by_test() {
  ...
  builder.filters
  |> list.find(fn(f) { f.0 == "order" })
  |> should.equal(Ok(#("order", "date", "desc")))
}
...
pub fn limit_test() {
  ...
  builder.filters
  |> list.find(fn(f) { f.0 == "limit" })
  |> should.equal(Ok(#("limit", "", "10")))
}
...
pub fn range_test() {
  ...
  builder.filters
  |> list.find(fn(f) { f.0 == "range" })
  |> should.equal(Ok(#("range", "", "0-9")))
}
...
pub fn complex_query_chain_test() {
  ...
  |> should.equal(Some("id, title, slug, category, date"))
...
pub fn update_operation_test() {
  let client = test_client()
  let updates = json.object([#("title", json.string("Updated Title"))])
...
pub fn in_filter_test() {
  ...
  builder.filters
  |> should.equal([#("in", "category", "(technology,tutorial,update)")])
}
...
pub fn rpc_call_test() {
  let client = test_client()
  let params =
    json.object([#("user_id", json.string("123")), #("limit", json.int(10))])
  ...
  case rpc("get_user_posts", params, client) {
    Ok(_) -> should.fail()
    // Shouldn't succeed without real endpoint
    Error(_) -> Nil
...
pub fn query_execution_integration_test() {
  ...
  case execute(query) {
    Ok(_response) -> {
      ...
      // In a real test we would decode this
      Nil
    }
    Error(error) -> {
...
pub fn update_with_filters_and_select_test() {
  let client = test_client()
  let updates = json.object([#("status", json.string("published"))])
