...
import supabase.{
  HttpRequestError, create, delete, eq, execute, from, insert, rpc, select,
  update,
}
...
pub fn update_sets_body_and_method_test() {
  let client = create("https://example.supabase.co", "test-key")
  let data = json.object([#("name", json.string("Jane Doe"))])
...
pub fn multiple_filters_test() {
  ...
  builder.filters
  |> should.equal([#("eq", "status", "active"), #("eq", "role", "admin")])
}
...
pub fn update_with_filter_test() {
  let client = create("https://example.supabase.co", "test-key")
  let data = json.object([#("status", json.string("archived"))])
