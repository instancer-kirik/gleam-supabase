...
pub fn run_query_test() {
  ...
  case result {
    Error(error_msg) -> {
      error_msg |> string.contains("HTTP client not implemented") |> should.be_true()
    }
...
pub fn insert_and_select_test() {
  ...
  let post_data = json.object([
    ...
    #("tags", json.preprocessed_array([json.string("tech"), json.string("gleam")])),
  ])
...
pub fn complex_json_insertion_test() {
  ...
  let complex_data = json.object([
    ...
    #("preferences", json.object([
      ...
      #("languages", json.preprocessed_array([
        json.string("english"),
        json.string("spanish"),
      ])),
    ])),
...
pub fn special_characters_test() {
  ...
  builder.select_columns |> should.equal(Some("first_name, last_name, email_address"))
}